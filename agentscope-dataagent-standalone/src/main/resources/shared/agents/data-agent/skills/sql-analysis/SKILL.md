...
2. **定位数据源（直接相关的表一次性 prepare）。** 确定数据在哪里：
   ...
   - 根据用户问题的指标，从 `[DATA_SOURCES_OVERVIEW]` 中挑选 **与问题直接相关的 1–3 张表**，用 `prepare_data_context` 的 `tables` 参数**一次调用批量取回**，确认列名、类型、描述和维度值样例——不要一张一张分开调用，也不要漏掉直接相关的表而盲写列名。
...
3. **不要发纯"摸底"查询。** `prepare_data_context` 返回的表描述和列信息已足够写正式查询。看完列结构后，**直接写回答用户问题的 SQL**——不要发 `SELECT COUNT(*)` / `SELECT MIN(date)` 等仅为了了解数据规模、日期范围的探查查询。 在 ```sql``` 代码块中写 SQL。约定：
   ...
   - 禁止把上一步查询结果作为字面量复制进 `IN (...)` 做跨表筛选——`row_limit` 截断会静默漏数，链式复制任一环出错即污染最终答案。
...
- ❌ 把上一步查询结果字面量复制进 `IN (...)` 做跨表筛选——改用 `JOIN` / CTE 让筛选条件留在库内。
