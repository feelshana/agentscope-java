...
import { Dataset, uploadInstructions } from '../api/datasets';
import { autoGenerateOntology, uploadOntology } from '../api/ontology';
...
export default function GenerateOntologyModal({
  ...
  const canConfirm = mode === 'tables' ? selected.length > 0 : ontologyFile !== null;
  ...
  async function confirm() {
    ...
    try {
      if (mode === 'tables') {
        await autoGenerateOntology(groupId, { tableNames: selected });
      } else {
        if (!ontologyFile) return;
        await uploadOntology(ontologyFile, groupId);
          ...
          {mode === 'tables' ? (
            <>
              <div className="da-small">
                从已有数据表生成独立对象目录和关系图；可勾选子集，普通 SQL 问数不依赖本体。
              </div>
            ...
            <>
              <div className="da-small">
                上传本体 YAML 文件（必选）；业务规则文档（可选）独立保存到当前知识库。
              </div>
              <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                />
                {ontologyFile && <span className="da-small">{ontologyFile.name}</span>}
              </label>
              <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <span style={{ fontSize: '0.82rem', color: 'var(--da-text-2)' }}>
