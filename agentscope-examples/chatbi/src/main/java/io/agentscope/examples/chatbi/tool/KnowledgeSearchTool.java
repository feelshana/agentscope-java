...
KnowledgeSearchTool {
    ...
    @Tool(
            name = "search_reports",
            description = "搜索报表或仪表盘。当用户询问某类报表、仪表盘名称或需要查找相关报表时使用此工具。"
                    + "参数 query 是用户的搜索关键词。")
    public Mono<String> searchReports(
            @ToolParam(name = "query", description = "用户的搜索关键词，例如：活跃用户报表")
            String query) {
    ...
    @Tool(
            ...
            description = "查询知识库中的指标定义、业务知识或系统知识。"
                    + "参数 query 是用户的问题；questionType 是问题类型：\"in\" 表示指标口径/定义查询，\"bu\" 表示业务知识/系统知识查询。")
    public Mono<String> queryKnowledge(
            @ToolParam(name = "query", description = "用户的问题")
            String query,
            @ToolParam(name = "questionType", description = "问题类型：\"in\" 表示指标口径/定义查询，\"bu\" 表示业务知识/系统知识查询")
            String questionType) {
    ...
    @Tool(
            ...
            description = "查询知识库（完整版），支持传入项目ID、当前报表/仪表盘名称和对话历史记忆以获得更精准的结果。"
                    + "query: 用户问题; questionType: \"in\"(指标口径) 或 \"bu\"(业务知识); "
                    + "projectId: 项目ID; reportName: 报表名; dashboardName: 仪表盘名; memory: 对话历史JSON字符串")
    public Mono<String> queryKnowledgeWithContext(
            ...
            String memoryJson) {
        log.info("[KnowledgeSearchTool] queryKnowledgeWithContext query={}, type={}, projectId={}",
                query, questionType, projectId);
