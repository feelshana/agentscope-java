...
public class ConfluenceFilterTool {
    ...
    private static final int FILTER_TIMEOUT_SECONDS = 2000;
    ...
    @Tool(
            ...
            description =
                    "Filter Confluence search results using AI to select the most relevant page IDs. "
                            + "Inputs: user_question (the original user question) and search_result (raw JSON from search_confluence). "
                            + "Output: comma-separated page IDs (up to 7). "
                            + "If no relevant pages found, returns default ID 437387755. "
                            + "Use this tool after search_confluence to identify the most relevant pages.")
    public Mono<String> filterResults(
        ...
        return filterSearchResults(userQuestion, searchResult)
                .onErrorResume(
                        e -> {
                            log.warn(
    ...
    private Mono<String> filterSearchResults(String userQuestion, String searchResult) {
        ...
        return filterModel
                .stream(messages, null, null)
                .collectList()
                .timeout(Duration.ofSeconds(FILTER_TIMEOUT_SECONDS))
